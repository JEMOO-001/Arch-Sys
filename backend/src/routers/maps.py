...
def check_map_authorization(current_user: TokenData, db_map: Map):
    ...
    if current_user.role == "analyst" and db_map.analyst_id != current_user.user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to access this record"
        )
...
async def create_map(
    ...
    # Log safe fields only (no file_path, no comment)
    safe_fields = {
        k: v
        for k, v in map_in.model_dump().items()
        if k not in ("file_path", "comment", "project_path")
    }
    logger.info(f"create_map request: {safe_fields}")
    ...
    logger.info(
        f"Creating map: unique_id={unique_id}, analyst_id={current_user.user_id}, category={dump.get('category')}, status={dump.get('status')}"
    )
    ...
    logger.info(
        f"Map created: id={db_map.map_id}, uid={db_map.unique_id}, category={db_map.category}, status={db_map.status}"
    )
...
async def list_my_maps(
    ...
    query = select(Map).where(
        Map.analyst_id == current_user.user_id, Map.tenant_id == tenant_id
    )
...
async def update_map(
    ...
    if current_user.role == "analyst" and db_map.analyst_id != current_user.user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to edit this record"
        )

    update_data = map_in.model_dump()  # Include all fields, even None
    # Log only safe fields (no comment)
    safe_update = {k: v for k, v in update_data.items() if k != "comment"}
    logger.debug(f"update_data = {safe_update}")
    ...
    if changes and current_user.user_id:
        changes_list = [
            f"{k}: {v[0]} -> {v[1]}" for k, v in changes.items() if v[0] != v[1]
        ]
        if changes_list:
            combined = "\n".join(changes_list)
...
async def reexport_map(
    ...
    for field, new_val in update_data.items():
        if new_val is not None:
            ...
            setattr(db_map, field, new_val)
            if field not in ("comment", "file_path"):
...
async def check_audit_logs_batch(
    ...
    result = await db.execute(
        ...
        .where(AuditLog.map_id.in_(map_ids), AuditLog.tenant_id == tenant_id)
        .distinct()
    )
