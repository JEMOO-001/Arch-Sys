...
import { 
  ArrowLeft, CheckCircle, MessageSquare, Clock, Send, 
  ShieldCheck, User, Calendar, Paperclip, X, Lock,
...
export const ReviewMode: React.FC<ReviewModeProps> = ({
  ...
  // Lock body scroll when preview overlay is active
  useEffect(() => {
    if (showPreview) {
  ...
  const handleSaveApproval = async () => {
    ...
    setIsSaving(true);
    try {
      await onUpdateApproval(record.map_id, approvalStatus, approvalComment);
      setApprovalComment('');
    } finally {
      setIsSaving(false);
    }
  ...
  const isSubmitDisabled = !approvalStatus || (
    (approvalStatus === 'Editing Required' || approvalStatus === 'On Hold') && !approvalComment.trim()
  );
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showPreview && (
          <motion.div 
            ...
            className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xl flex items-center justify-center p-6 md:p-12 overflow-hidden"
            style={{ marginTop: 0 }}
          >
            {/* Close Button top-right of screen */}
            <button 
              onClick={() => setShowPreview(false)}
              className="fixed top-4 right-4 z-50 bg-black/40 text-white hover:bg-black/60 rounded-full p-3 transition-all cursor-pointer shadow-lg border border-white/10"
              style={{ marginTop: 0 }}
              title="Close Preview (ESC or Click Outside)"
            >
              <X className="h-6 w-6" />
            </button>
            ...
            >
              ...
              <div className="w-full h-full overflow-hidden relative flex items-center justify-center">
                ...
                {previewError && (
                  <div className="flex h-full flex-col items-center justify-center text-red-400 p-4 bg-gray-900/40 rounded-xl">
                    <FileText className="h-16 w-16 mb-4 opacity-80" />
                    <p className="font-bold text-lg">Failed to load preview</p>
                    <p className="text-sm opacity-60 mt-1 max-w-md text-center">{previewError}</p>
                  </div>
                )}
                {previewFileUrl && !loadingPreview && !previewError && previewFileType === 'pdf' && (
                  <iframe
                    src={`${previewFileUrl}#toolbar=0&navpanes=0&scrollbar=0&view=Fit`}
                    className="h-full w-full border-0 bg-white rounded-xl shadow-2xl block"
                    style={{ margin: 0, padding: 0 }}
                    title="Map Preview"
                  />
                )}
                {previewFileUrl && !loadingPreview && !previewError && previewFileType === 'image' && (
                  <img
                    src={previewFileUrl}
                    alt="Map Preview"
                    className="max-h-full max-w-full object-contain shadow-2xl rounded-xl block"
                  />
                )}
              </div>
            </motion.div>
      ...
      >
        {/* Header Bar */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-xl border border-gray-200 shadow-sm gap-4">
          ...
          <div className="flex items-center justify-between sm:justify-end gap-4">
            <Button
              ...
              className="flex items-center gap-2 border border-gray-200 hover:bg-gray-50 text-gray-700 font-bold py-2.5 px-4"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className={`bg-white rounded-xl border-2 p-6 shadow-sm transition-colors duration-300 ${isLocked ? 'border-green-500 bg-green-50/10' : 'border-blue-100'}`}>
              ...
              <div className="space-y-5">
                      ...
                      {approvalStatus && (
                        <div className="space-y-1.5 mt-3 animate-fade-in">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">
                            Decision Comment {(approvalStatus === 'Editing Required' || approvalStatus === 'On Hold') && <span className="text-red-500">*</span>}
                          </label>
                          <textarea
                            ...
                            placeholder={
                              approvalStatus === 'Approve'
                                ...
                                : 'Please describe the required changes or reason for hold (required)...'
                            }
                            className="w-full rounded-lg border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-blue-500 transition-all resize-none text-gray-700 font-medium"
                            rows={3}
                          />
                        </div>
                      )}
                ...
                {isAdmin && (
                  <Button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:shadow-none text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-100 transition-all"
                  >
